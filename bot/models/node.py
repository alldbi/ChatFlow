...
class Node(FlowItem):
    def __init__(
        ...
        next_item: Union[
            FlowItem, List[FlowItem], Dict[FlowItem, Condition], None
        ] = None,
        ...
        self.next: Union[FlowItem, List[FlowItem], Dict[FlowItem, Condition]] = (
            next_item
        )
...
class JsonOutputNode(Node):
    def __init__(
        ...
        self.model = llm.with_structured_output(
            self._get_output_model(), method="json_mode", include_raw=True
        )
    ...
    def run(self, inp: Dict):
        ...
        print("jsonOut", output)
        return output

    def _get_output_model(self):
        output_model = create_model(
            "OutputModel", **{k: (v, ...) for k, v in self.output_variables.items()}
        )
        return output_model
...
class RetrievalStrOutputNode(StrOutputNode):
    ...
    def run(self, inp: Dict):
        ...
        #     query=inp[self.query_var], k=self.k
        # )
...
class RetrievalNode(Node):
    def __init__(
        ...
        self.node: Union[StrOutputNode, JsonOutputNode] = NodeFactory.create_node(
            model_name,
            prompt_template,
            input_variables,
            output_variables,
